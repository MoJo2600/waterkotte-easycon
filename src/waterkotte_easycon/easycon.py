...
class EasyCon:
    ...
    @staticmethod
    def set_value(values, idx, value):
    ...
    @staticmethod
    def parse_I1_firmware(values, idx, value):
        ...
        EasyCon.set_value(values, idx, f"{major:02}.{minor:02}.{patch:02}")
    ...
    def __init__(self, url: str, user: str = 'waterkotte', password: str = 'waterkotte', session: ClientSession | None = None, hp_types_file: str | None = None, xml_data_file: str | None = None) -> None:
        ...
        # Generate file to create this map?
        self._analog_variables = {
            'A25': {
                'unit': 'kWh',
                'description': 'ElectricalPower'
            },
            'A26': {
                'unit': 'kWh',
        ...
        if hp_types_file is not None:
            self._load_hp_types(hp_types_file)

        if xml_data_file is not None:
            self.parse_xml_data(xml_data_file)

        # http://192.168.158.203/http/easycon/hpType.csv

    def parse_xml_data(self, xml_data_file):
        """
        Load XML data from file
        """
        with open(xml_data_file, 'r') as xml_file:
            section: str = None
                ...
                if event == 'end':
                    ...
                    if elem.tag == 'VARIABLE':
                        ...
                        if index in self._parse_methods:
                            self._parse_methods[index](self._analog_variables, index, value)
                        elif section == 'A':
                            EasyCon.set_analog_value(self._analog_variables, index, value)
                        elif section == 'D':
                            EasyCon.set_digital_value(self._analog_variables, index, value)
                        elif section == 'I':
                            EasyCon.set_integer_value(self._analog_variables, index, value)

        # for key in self._analog_variables.keys():
        #     print(f"{key=} : {self._analog_variables[key]['value']}")

    def _load_hp_types(self, hp_types_file):
        """
        Load hp types from file
        """
        self._aI110_ids = []
        self._aI105_series = []
        with open(hp_types_file, mode='r') as file:
            ...
            for lines in csvFile:
                self._aI105_series.append(lines[2])
                self._aI110_ids.append(lines[1])
